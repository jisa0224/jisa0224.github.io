...
4.  Install Custom System: 選這個才會安裝圖形相關的軟體包，Install CLI System 不會有
    1.  Install Base Packages
        *   Install Base: 選 `linux510`  
            (如果選 `linux-latest` 的話，在當前版本 EOL 後會自動更新到最新版本，測試用機可以選這個；但正式用機最好是手動更新，可以保留舊版以避免無法啟動時作為備援)
...
9.  在 `~/.pam_environment` 加入 `QML_DISK_CACHE_PATH=/tmp/QtQmlDiskCache_jisa`，重開機後用 tty 刪除 `~/.cache` 後再登入。  
    這樣可以避免 Qt 在 `~/.cache` 裡放一大堆 QML 的快取（即使是不再使用的 QML 程式，它也不會自己刪除快取！），雖然重開機後第一次啟動 QML 程式會比較慢，但是之後因為快取位於 tmpfs 上，反而會比較快。  
    如果完全不要快取，改成加入 `QML_DISABLE_DISK_CACHE=1`。  
...
由於 root 沒有這些 Bash 初始化 script，所以 `sudo -i` 出來的結果是黑白的，`EDITOR` 也不對，所以執行 `sudo ln -s ~jisa/{.bash_profile,.bashrc} /root`
...
* jre-openjdk  
  修正字型很丑的問題：在 `~/.pam_environment` 加入 `_JAVA_OPTIONS=-Dawt.useSystemAAFontSettings=on`。參考資料：[Java Runtime Environment fonts - ArchWiki](https://wiki.archlinux.org/index.php/Java_Runtime_Environment_fonts#Overriding_the_automatically_picked_up_settings)、[environment variables - Where can I set global Java Options? - Unix & Linux Stack Exchange](https://unix.stackexchange.com/questions/151733/where-can-i-set-global-java-options)
