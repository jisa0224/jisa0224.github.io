...
如果是要安裝到 VirtualBox 作為 guest，參考[這篇](manjaro-02-as-virtualbox-guest.md)。
...
1.  Prepare Installation
    ...
    3.  Partition Disk: 使用 `parted` 建立分割區(gparted 只分割，不格式化)，如果已經分割過了就不用
        ```
        # nvme0n1
        mktable gpt
...
4.  Install Custom System: 選這個才會安裝圖形相關的軟體包，Install CLI System 不會有
    1.  Install Base Packages
        *   Install Base: 選 `linux-latest` 和 `linux-lts`  
            (`linux-latest` 是最新版的核心，在當前版本 EOL 後會自動更新到最新版本；`linux-lts` 是最新版的 LTS 核心，作為救援備用)
...
* 顯示 GRUB 開機選單及輸出開機資訊  
  編輯 `/etc/default/grub`  
  (1) 修改 `GRUB_TIMEOUT`  
  (2) 刪除 `GRUB_TIMEOUT_STYLE`  
  (3) `GRUB_CMDLINE_LINUX_DEFAULT` 刪除 `quiet`  
  儲存後執行 `sudo update-grub`。

* 修正[新電腦的 NVMe SSD 問題](manjaro-05-new-computer-notice.md)  
  編輯 `/etc/default/grub`，在 `GRUB_CMDLINE_LINUX_DEFAULT` 加上 `nvme_core.default_ps_max_latency_us=0`，儲存後執行 `update-grub`。

* 調整 sudo timeout，讓 sudo 不用隔一段時間重新輸入密碼。  
  執行 `sudo EDITOR=vi visudo -f /etc/sudoers.d/$USER`，加入 `Defaults timestamp_timeout=-1` 後儲存離開。

* 系統安裝完後，systemd 的藍牙服務未開啟，`sudo systemctl enable bluetooth` 設定開機後自動啟動藍牙服務，`sudo systemctl start bluetooth` 立即啟動藍牙服務。
...
* 更新 pacman 使用的鏡像伺服器  
  執行 `sudo pacman-mirrors --country Taiwan,Japan,United_States,United_Kingdom,France,Germany && sudo pacman -Syyu`
...
* 語言設定為 "繁體中文（台灣）"  
  ...
  到 System Settings > Locale 加入 "zh_TW.UTF-8"，然後按右鍵選 "Set as default display language and format"（註：這裡設定的內容即為 `locale` 指令顯示的內容）  
  然後重開機

* Manjaro 對 KDE 的客製化(設定工具與主題)  
  安裝 `manjaro-kde-settings sddm-breath2-theme manjaro-settings-manager-knotifier manjaro-settings-manager-kcm`  
  然後到 System Settings > Global Theme 選擇 "Breath2"  
  另外到 System Settings > Startup & Shutdown > Login Screen (SDDM) 選擇 "Breath2"

* 到 System Settings > Search 關閉搜尋功能（因為它佔用太多 CPU 和 RAM，此功能由 baloo 提供）

* 關閉 KDE Wallet subsystem 以避免它一直跳出視窗要求輸入密碼
  在 `~/.config/kwalletrc` 中加入以下內容
  ```
  [Wallet]
  Enabled=false
  ```

* 由於 KDE Plasma 的桌面沒有捷徑功能，必須依靠 `.desktop` 的方式實現。  
  在 `~/Desktop` 新增以下三個檔案
  ``` 
  # root.desktop
  [Desktop Entry]
...
把字型檔放到 `~/.local/share/fonts`，fontconfig 設定檔放到 `~/.config/fontconfig/fonts.conf`。

這個 fontconfig 設定檔非常重要，如果沒有它，放進去的字型檔會把預設的配置搞的亂七八糟。
...
**重要**：把 manjaro-kde-settings 新增的 `/etc/xdg/plasma-workspace/env/envars.sh` 中同名的環境變數註解掉，不然它會在 KDE Plasma 桌面環境中覆蓋掉 `/etc/environment` 的環境變數。

為了避免每次更新 manjaro-kde-settings 後都要手動修改此檔案，可以利用 libalpm 的 hook。新增 `/usr/share/libalpm/hooks/manjaro-kde-settings-disable-envars.hook` 並輸入以下內容：
...
Operation = Upgrade
Type = Path
Target = etc/xdg/plasma-workspace/env/envars.sh

[Action]
Description = Disable environment variables exported by manjaro-kde-settings
When = PostTransaction
Exec = /usr/bin/sed -i -r -e 's/^(export)\ (EDITOR|BROWSER)/# \1 \2/' /etc/xdg/plasma-workspace/env/envars.sh
Depends = sed
```
...
* `~/.bashrc`: The individual per-interactive-shell startup file
  ...
  # Linux console (virtual terminal, VT) doesn't support CJK
  if [[ "$TERM" == "linux" ]]; then
...
* 由於 root 沒有這兩個檔，所以 `sudo -i` 出來的結果是黑白的，執行 `sudo ln -s ~jisa/{.bash_profile,.bashrc} /root`。
...
* 不含在 "Manjaro Architect" 和 "系統設定" 安裝的軟體包
* 未特別注記來源者，代表來自 Manjaro 官方庫
...
* konsole  
  配色 "微風"，背景透明 "10%"，字型 "Noto Mono 12pt"
* dolphin  
...
* google-chrome<sup>AUR</sup>
    ...
    * 使用久了之後會儲存大量的快取，"清除瀏覽資料" 裡的 "Cookie 和其他網站資料" 可以清除 `~/.config/google-chrome` 裡的快取，"快取圖片和檔案" 可以清除 `~/.cache/google-chrome` 裡的快取。
* JDownloader 2<sup>其它</sup>(dep: jre-openjdk)
...
* python-numpy python-matplotlib(dep: pyside2) python-scipy
...
* virtualbox(dep: linux-latest-virtualbox-host-modules, linux-lts-virtualbox-host-modules) virtualbox-guest-iso virtualbox-ext-oracle<sup>AUR</sup>
...
* fcitx5 fcitx5-qt fcitx5-gtk fcitx5-configtool fcitx5-chewing
* tree htop lsof
* pamac-cli pamac-gtk
* timeshift
* gufw
...
* fcitx5  
  ...
  SDL_IM_MODULE=fcitx
  ```
  安裝後 fcitx5 會自動被加到 /etc/xdg/autostart 裡，所以可以自動啟動。若沒有自動啟動可以執行 `ln -s /usr/share/applications/org.fcitx.Fcitx5.desktop ~/.config/autostart/`。  
...
* gufw
  安裝後執行 `sudo systemctl enable --now ufw`，然後才用圖形界面 gufw 開啟防火牆。
  警告：不能同時啟用 `ufw` 和 `iptables/ip6tables` 服務。
* baobab  
  mate-disk-usage-analyzer 被包在 `mate-utils` 裡，`mate-utils` 又有太多其它東西，所以不採用
* jre-openjdk  
  修正字型很丑的問題：在 `/etc/environment` 加入 `_JAVA_OPTIONS=-Dawt.useSystemAAFontSettings=on`。參考資料：[Java Runtime Environment fonts - ArchWiki](https://wiki.archlinux.org/index.php/Java_Runtime_Environment_fonts#Overriding_the_automatically_picked_up_settings)、[environment variables - Where can I set global Java Options? - Unix & Linux Stack Exchange](https://unix.stackexchange.com/questions/151733/where-can-i-set-global-java-options)
...
* Qt 會在 `~/.cache` 裡放一大堆 QML 的快取，但即使是不再使用的 QML 程式，它也不會自己刪除快取！  
  可以在 `/etc/environment` 加入 `QML_DISK_CACHE_PATH=/tmp/QtQmlDiskCache_jisa`，把快取放到 tmpfs 上，每次關機自動清理，雖然重開機後第一次啟動 QML 程式會比較慢(其實感覺不太出來)，但是之後因為快取位於 tmpfs 上，反而會比較快；或是改成加入 `QML_DISABLE_DISK_CACHE=1`，完全關閉快取。  
